...
> {
  ...
  // Object schemas are not discrete / numeric distributions; calling these
  // modifiers is a programming error. We throw at schema-build time rather
  // than silently ignoring at sample time so the failure surfaces in the
  // unit test that constructs the schema, not in production telemetry.
  /** @deprecated `.weighted(...)` is not supported on `obj(...)`. Apply it to the discrete field instead. */
  override weighted(_weights: ReadonlyArray<readonly [string | number | boolean, number]>): this {
    throw new SchemaConflictError(
      'obj: .weighted(...) is not supported on object schemas.',
      [],
      'attach `.weighted([...])` to the specific discrete field that should be biased.',
    )
  }
  /** @deprecated `.normal(...)` is not supported on `obj(...)`. */
  override normal(_mean: number, _stddev: number): this {
    throw new SchemaConflictError(
      'obj: .normal(...) is not supported on object schemas.',
      [],
      'attach `.normal(mean, stddev)` to the specific numeric field instead.',
    )
  }
  /** @deprecated `.typically(...)` is not supported on `obj(...)`. */
  override typically(_from: number, _to: number): this {
    throw new SchemaConflictError(
      'obj: .typically(...) is not supported on object schemas.',
      [],
      'attach `.typically(from, to)` to the specific numeric field instead.',
    )
  }
}
...
export class ArraySchema<S extends Schema> extends Schema<Infer<S>[]> {
  ...
  // ── capability matrix — see docs/STABILITY.md ───────────────────────────
  /** @deprecated `.weighted(...)` is not supported on `arr(...)`. Apply it to the element schema instead. */
  override weighted(_weights: ReadonlyArray<readonly [string | number | boolean, number]>): this {
    throw new SchemaConflictError(
      'arr: .weighted(...) is not supported on array schemas.',
      [],
      'attach `.weighted([...])` to the element schema (`arr(int().weighted([...]))`).',
    )
  }
  /** @deprecated `.normal(...)` is not supported on `arr(...)`. */
  override normal(_mean: number, _stddev: number): this {
    throw new SchemaConflictError(
      'arr: .normal(...) is not supported on array schemas.',
      [],
      'attach `.normal(mean, stddev)` to the numeric element schema instead.',
    )
  }
  /** @deprecated `.typically(...)` is not supported on `arr(...)`. */
  override typically(_from: number, _to: number): this {
    throw new SchemaConflictError(
      'arr: .typically(...) is not supported on array schemas.',
      [],
      'attach `.typically(from, to)` to the numeric element schema instead.',
    )
  }
}
...
export class TupleSchema<T extends readonly Schema[]> extends Schema<InferTuple<T>> {
  ...
  // ── capability matrix — see docs/STABILITY.md ───────────────────────────
  /** @deprecated `.weighted(...)` is not supported on `tuple(...)`. Apply it to each element. */
  override weighted(_weights: ReadonlyArray<readonly [string | number | boolean, number]>): this {
    throw new SchemaConflictError(
      'tuple: .weighted(...) is not supported on tuple schemas.',
      [],
      'attach `.weighted([...])` to the specific tuple element schema(s) you want biased.',
    )
  }
  /** @deprecated `.normal(...)` is not supported on `tuple(...)`. */
  override normal(_mean: number, _stddev: number): this {
    throw new SchemaConflictError(
      'tuple: .normal(...) is not supported on tuple schemas.',
      [],
      'attach `.normal(mean, stddev)` to the specific numeric tuple element instead.',
    )
  }
  /** @deprecated `.typically(...)` is not supported on `tuple(...)`. */
  override typically(_from: number, _to: number): this {
    throw new SchemaConflictError(
      'tuple: .typically(...) is not supported on tuple schemas.',
      [],
      'attach `.typically(from, to)` to the specific numeric tuple element instead.',
    )
  }
}
...
export class UnionSchema<T extends readonly Schema[]> extends Schema<InferUnion<T>> {
  ...
  // ── capability matrix — see docs/STABILITY.md ───────────────────────────
  // The historical `.weighted()` on `union(...)` silently sampled
  // uniformly because the generator never read the distribution axis
  // for unions. Pretending to accept the bias was strictly worse than
  // failing loud: callers thought they had shaped the distribution.
  /** @deprecated `.weighted(...)` is not supported on `union(...)`. Use `discriminated(...)`. */
  override weighted(_weights: ReadonlyArray<readonly [string | number | boolean, number]>): this {
    throw new SchemaConflictError(
      'union: .weighted(...) is not supported on union schemas.',
      [],
      'replace `union(a, b).weighted([...])` with `discriminated(key, { tagA: ..., tagB: ... })` for tag-driven branch selection.',
    )
  }
  /** @deprecated `.normal(...)` is not supported on `union(...)`. */
  override normal(_mean: number, _stddev: number): this {
    throw new SchemaConflictError(
      'union: .normal(...) is not supported on union schemas.',
      [],
      'attach `.normal(mean, stddev)` to the specific numeric branch instead.',
    )
  }
  /** @deprecated `.typically(...)` is not supported on `union(...)`. */
  override typically(_from: number, _to: number): this {
    throw new SchemaConflictError(
      'union: .typically(...) is not supported on union schemas.',
      [],
      'attach `.typically(from, to)` to the specific numeric branch instead.',
    )
  }
}
