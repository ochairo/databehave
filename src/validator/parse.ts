...
const check = (
  ...
  if (value === undefined) {
    ...
    issues.push({ path, message: 'required', expected: describe(node) })
    return undefined
  }
  if (value === null) {
    if (mods?.nullable === true) return null
    if (node.kind === 'null') return null
    issues.push({ path, message: 'unexpected null', expected: describe(node), received: null })
    return null
  }
  ...
  switch (node.kind) {
    case 'string': {
      if (typeof value !== 'string') {
        issues.push({ path, message: 'expected string', expected: 'string', received: value })
        return value
      }
      if (node.min !== undefined && value.length < node.min) {
        issues.push({ path, message: `length < min ${node.min}`, received: value })
      }
      if (node.max !== undefined && value.length > node.max) {
        issues.push({ path, message: `length > max ${node.max}`, received: value })
      }
      if (node.pattern !== undefined && !new RegExp(node.pattern).test(value)) {
        issues.push({ path, message: `does not match /${node.pattern}/`, received: value })
      }
      return value
    }
    case 'number': {
      if (typeof value !== 'number' || Number.isNaN(value)) {
        issues.push({ path, message: 'expected number', expected: 'number', received: value })
        return value
      }
      if (node.int && !Number.isInteger(value)) {
        issues.push({ path, message: 'expected integer', received: value })
      }
      if (node.min !== undefined && value < node.min) {
        issues.push({ path, message: `< min ${node.min}`, received: value })
      }
      if (node.max !== undefined && value > node.max) {
        issues.push({ path, message: `> max ${node.max}`, received: value })
      }
      return value
    }
    case 'decimal': {
      if (typeof value !== 'string') {
        issues.push({ path, message: 'expected decimal string', expected: 'string', received: value })
        return value
      }
      if (!/^-?\d+(\.\d+)?$/.test(value)) {
        issues.push({ path, message: 'not a numeric string', received: value })
      } else {
    ...
    case 'boolean': {
      if (typeof value !== 'boolean') {
        issues.push({ path, message: 'expected boolean', expected: 'boolean', received: value })
      }
      return value
    }
    case 'null': {
      if (value !== null) {
        issues.push({ path, message: 'expected null', expected: 'null', received: value })
      }
      return value
    }
    case 'literal': {
    ...
    case 'array': {
      if (!Array.isArray(value)) {
        issues.push({ path, message: 'expected array', expected: 'array', received: value })
        return value
      }
      ...
      if (node.minLength !== undefined && value.length < node.minLength) {
        issues.push({ path, message: `length < minLength ${node.minLength}` })
      }
      if (node.maxLength !== undefined && value.length > node.maxLength) {
        issues.push({ path, message: `length > maxLength ${node.maxLength}` })
      }
      return value.map((item, i) => check(node.item, item, [...path, i], issues, root, parent))
    }
    case 'object': {
      if (typeof value !== 'object' || Array.isArray(value)) {
        issues.push({ path, message: 'expected object', expected: 'object', received: value })
        return value
      }
    ...
    case 'tuple': {
      if (!Array.isArray(value)) {
        issues.push({ path, message: 'expected tuple (array)', received: value })
        return value
      }
