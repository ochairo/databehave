...
**Modifier short-circuits are opt-in.** `optional`, `nullable`, and
`default` do **not** inject randomness during `mock()` by default; the
field is always sampled. Pass `MockOptions.modifierProbs = { optional:
0.1, nullable: 0.1, default: 0.2 }` (or any subset) to exercise those
paths probabilistically.

`expectStable(schema, opts)` is the executable form of this contract:
it generates twice and throws on divergence. Use it in regression tests
to catch accidental non-determinism in user `derivedFrom` callbacks.
