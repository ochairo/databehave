...
describe('ConformError', () => {
  it('is a real Error subclass with name="ConformError"', () => {
    const e = new ConformError([{ path: ['x'], code: 'required', message: 'bad' }])
  ...
  it('renders the root path as "(root)"', () => {
    const e = new ConformError([{ path: [], code: 'required', message: 'wrong' }])
    assert.match(e.message, /\(root\)/)
  })

  it('renders nested paths as dot-joined strings', () => {
    const e = new ConformError([{ path: ['user', 'id'], code: 'required', message: 'bad' }])
    assert.match(e.message, /user\.id/)
  })

  it('preserves array index segments in path', () => {
    const e = new ConformError([{ path: ['items', 2, 'name'], code: 'required', message: 'bad' }])
  ...
  it('safeStringify falls back to String(v) for values that cannot be JSON-encoded', () => {
    ...
    const e = new ConformError([{ path: [], code: 'required', message: 'bad', received: circular }])
  ...
  it('omits "expected=" and "received=" when not provided', () => {
    const e = new ConformError([{ path: ['x'], code: 'required', message: 'plain' }])
